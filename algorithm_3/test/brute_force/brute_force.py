...
def pair_dig_dump(matrix=None):
    ...
    # Return all possible parings between a dig locations and available dump locations.
    ...
    def all_pairs_all_digs(dig_dump_pairs):
        ...
        for dig_location in dig_locations:
            ...
            #print('\n',pairs, dig_location)
            for pair in pairs:
                # Save current progress for next recursive branch.
                sequence = {dig_location: dig_dump_pairs[-1][dig_location] for dig_location in dig_dump_pairs[-1]}

                # Count the actions required for each pairing.
                # Level
                dig_counts = []
                for dump_location in pair: # dig
                    dig_counts.append(dig_and_dump(dig_location, dump_location))

                # Assign dump locations and actions_required  to dig_location.
                dig_dump_pairs[-1][dig_location] = [(dump_location, actions_required) for dump_location, actions_required in zip(pair, dig_counts)]# pair dig locations and dump locations
                ...
                # Fast forward next branch.
                dig_dump_pairs.append(sequence)
        # Remove las fast forward.
        if valid:
